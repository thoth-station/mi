...
class Report:
    ...
    def ttr_in_time(analysed_entities):
        ...
        projects_reviews_data = processing.processing_prs_project_data(data=analysed_entities)
    ...
    def ttci_in_time(analysed_entities):
        ...
        project_issues_data = processing.processing_issues_project_data(data=analysed_entities)
    ...
    def top_x_contributors_activity(issues, prs, x):
        issue_creators = processing.processing_issues_creators(issues)
        pr_creators = processing.processing_issues_creators(prs)
        issue_closers = processing.processing_issues_closers(issues, prs)
        # pr_reviews = processing.processing_pr_reviews(prs)
    ...
    def health_report(self, app):
        ghs = KnowledgeStorage(is_local=True)
        ...
        pr_creators = self.viz.processing.processing_issues_creators(prs)
        issue_creators = self.viz.processing.processing_issues_creators(issues)
    ...
    def update_opener(value):
        ghs = KnowledgeStorage(is_local=True)
        issues = ghs.load_previous_knowledge(project_name='AICoE/prometheus-anomaly-detector', knowledge_type='Issue')
        overall = processing.processing_issue_labels_to_issue_creators(
                issues_data=issues
        )
    ...
    def update_inter(value):
        ghs = KnowledgeStorage(is_local=True)
        issues = ghs.load_previous_knowledge(project_name='AICoE/prometheus-anomaly-detector', knowledge_type='Issue')
        overall_issues_interactions = processing.processing_issue_interactions(issues_data=issues)
