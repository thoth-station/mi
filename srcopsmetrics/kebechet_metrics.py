...
class KebechetMetrics:
    ...
    def evaluate_and_store_kebechet_metrics(self, is_local: bool):
        ...
        for get_stats in [self.update_manager]:
            stats = get_stats()
            path = Path(f"./srcopsmetrics/metrics/{self.repo_name}/kebechet_{get_stats.__name__}.json")
            KnowledgeStorage(is_local=is_local).save_knowledge(file_path=path, data=stats)
