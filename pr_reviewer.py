...
def evaluate_reviewers(
    ...
    if source_knowledge_file.exists():
        ...
        with open(source_knowledge_file, "r") as fp:
            ...
            team_results = knowledge_results_repo.joinpath(f"MTTR-in-time-{project[1] + '-' + project[0]}.png")
            fig.savefig(team_results)
            plt.close()

            ##############################################################################
            if use_median:
                MTTR = np.median([pr["PR_TTR"] for pr in data["results"].values() if pr["PR_approved"]])
            else:
                MTTR = np.mean([pr["PR_TTR"] for pr in data["results"].values() if pr["PR_approved"]])

            total_number_commits = sum([pr["PR_commits_number"] for pr in data["results"].values()])
            total_number_PR = len([pr for pr in data["results"].values()])
            Number_PR_reviewed = len([pr for pr in data["results"].values() if pr["PR_approved"]])
            _LOGGER.info(
                "-----------------------------------------------------------------------------------------------------------------------------------------------"
            )
            _LOGGER.info(
            ...
            _LOGGER.info(
                "{:40} --> {:^12} |{:^18} | {:^9} | {:^26} |".format(
                    project[1] + "/" + project[0], total_number_PR, total_number_commits, Number_PR_reviewed, str(timedelta(seconds=MTTR))
                )
            )
            _LOGGER.info(
                "-----------------------------------------------------------------------------------------------------------------------------------------------"
            ...
            team_results = knowledge_results_repo.joinpath(f"{project[1] + '-' + project[0]}.png")
            fig.savefig(team_results)
            plt.close()
            ...
            for author in authors:
                ...
                total_number_commits_author = sum([pr["PR_commits_number"] for pr in data["results"].values() if pr["PR_author"] == author])
...
if __name__ == "__main__":
    if not PROJECTS:
        _LOGGER.warning("Please insert one project in PROJECTS in create_bot_knowledge.py file.")
